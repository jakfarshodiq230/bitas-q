                                    ...
                                    <form id="dataForm"
										...
										<div class="row">
											<div class="mb-3 col-md-4">
												<label for="inputEmail4">Rapor</label>
												<select class="form-control select2 mb-4 me-sm-2 mt-0"
                                                        name="kegiatan" data-bs-toggle="select2" required>
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $.ajax({
                url: '{{ url('admin/periode_rapor/data_tahun') }}',
                type: 'GET',
                dataType: 'json', // Ensure response is treated as JSON
                success: function(response) {
                    const data = response.data; // Assuming response has a 'data' array
                    const tahun_list = document.querySelector('select[name="kegiatan"]');
                    data.forEach(item => {
                        const option = document.createElement('option');
                        option.value = item.id_periode;
                        option.textContent =  item.jenis_periode.toUpperCase() + ' ' +item.nama_tahun_ajaran + ' ' + (item.jenis_kegiatan ? item.jenis_kegiatan.toUpperCase() : '');;
