                                    ...
                                    <form id="dataForm"
										...
										<div class="row">
											<div class="mb-3 col-md-4">
                                                ...
                                                <option value="" disabled selected>PILIH PERODE PENILAIAN</option>
                                            </select>
											</div>
                                            <div class="mb-3 col-md-4">
												<label for="inputEmail4">Taggal Akhir Penilaian Rapor</label>
												<input type="datetime-local" class="form-control" name="tggl_akhir_penilaian" id="tggl_akhir_penilaian" placeholder="tanggal">
                                                <input type="text" class="form-control" name="id_periode_rapor" id="id_periode_rapor" placeholder="id rapor" hidden>
											</div>
                                            <div class="mb-3 col-md-4">
												<label for="inputEmail4">Taggal Rapor</label>
												<input type="date" class="form-control" name="tggl_periode" id="tggl_periode" placeholder="tanggal">
											</div>
										</div>
                                        <div class="row">
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const selectedPeriode = '{{ isset($existingData) ? $existingData->id_periode : '' }}'; 
            const tahunList = document.querySelector('select[name="kegiatan"]');
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    data.forEach(item => {
                        ...
                        option.textContent = 'BINA PRIBADI ISLAM (BPI) ' + item.nama_tahun_ajaran + ' ' + (item.jenis_kegiatan ? item.jenis_kegiatan.toUpperCase() : '');
                        
                        // Check if this option should be pre-selected
                        if (item.id_periode === selectedPeriode) {
                            option.selected = true; // Set the option as selected
                        }

                        // Append the option to the select element
                        tahunList.appendChild(option);
                    });

                    // Initialize Select2 after appending options
                    $(tahunList).select2();

                    // Trigger change event in case of pre-selected value
                    if (selectedPeriode) {
                        $(tahunList).val(selectedPeriode).trigger('change');
                        btnSave.disabled = true;
                    }
                },
        ...
        $(document).ready(function() {
            ...
            $('#datatables-ajax').DataTable({
                ...
                destroy: true,
                responsive: true,
                ajax: '{{ url('admin/periode_rapor_pbi/data_periode_rapor_pbi') }}',
                columns: [{
                        "data": null,
                        "name": "rowNumber",
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            'Rapor : BINA PRIBADI ISLAM (BPI)' +' '+ jenis_kegiatan +
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return `<span class="badge ${new Date(row.tggl_akhir_penilaian) < new Date() ? 'bg-danger' : 'bg-success'}">Akhir Penilaian : ${tanggal_formatted}
                            </span> <br>
            ...
            Swal.fire({
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#dataForm input[name="id_periode_rapor"]').val(data.data.id_periode);

                        $('select[name="kegiatan"] option').each(function() {
                            if ($(this).val() === data.data.id_penilaian_periode) {
                                $(this).prop('selected', true);
                            }
                        });
                        $('select[name="kegiatan"]').select2().trigger('change');
        ...
        $('#saveBtn').on('click', function() {
            var id = $('#id_periode_rapor').val();
            var url = '{{ url('admin/periode_rapor_pbi/store_periode_rapor_pbi') }}';

            if (id) {
                url = '{{ url('admin/periode_rapor_pbi/update_periode_rapor_pbi') }}/' + id;
            }
            var form = $('#dataForm')[0];
        ...
        $(document).on('click', '.deleteBtn', function() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: '{{ url('admin/periode_rapor_pbi/delete_periode_rapor_pbi') }}/' +
        ...
        $(document).on('click', '.updateBtn1, .updateBtn0', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ url('admin/periode_rapor_pbi/status_periode_rapor_pbi') }}/' + id + '/' + status,
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
        ...
        $(document).on('click', '.pesertaBtn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '{{ url('admin/periode_rapor_pbi/peserta_periode_rapor_pbi') }}/' + tahun + '/' + rapor + '/' + periode,
                        type: 'GET',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
