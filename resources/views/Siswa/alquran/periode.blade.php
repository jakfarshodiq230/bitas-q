    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#datatables-ajax').DataTable({
                ...
                destroy: true,
                responsive: true,
                ajax: '{{ url('siswa/kegiatan/ajax-data-periode') }}',
                columns: [{
                        "data": null,
                        "name": "rowNumber",
                        ...
                        render: function(data, type, row) {
                            return `
                                ...
                                <button class="btn btn-sm btn-warning downloadBtn me-1" 
                                        ...
                                        data-id_periode="${row.id_periode}" 
                                        data-jenjang="${row.jenis_penilaian_kegiatan}"
                                        ${new Date(row.tggl_akhir_penilaian) > new Date() ? 'disabled' : ''}>
        ...
        $(document).on('click', '.lihatBtn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ url('siswa/kegiatan/detail-nilai') }}/" +
                    id_periode;
        ...
        $(document).on('click', '.downloadBtn', function() {
            ...
            var url= '{{ url('siswa/kegiatan/downloadRapor') }}/'+ id_periode + "/" + jenjang;
            window.location.href = url;
        });
    </script>
@endsection
