    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#datatables-ajax').DataTable({
                ...
                destroy: true,
                responsive: true,
                ajax: '{{ url('guru/penilaian_pbi/data_periode_penilaian_kegiatan') }}/'+guru,
                columns: [{
                        "data": null,
                        "name": "rowNumber",
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            + (row.status_periode === 1 
                                ...
                                : '<span class="badge bg-danger me-1">TIDAK AKTIF</span> ')
                            + ' <span class="badge bg-warning">'+row.sesi_periode + ' KALI/PEKAN </span>';
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <button class="btn btn-sm btn-secondary lihatBtn me-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Tambah Peserta" ${row.total_peserta_pbi === 0 ? 'disabled' : ''} data-id_periode="${row.id_periode}" data-tahun_ajaran="${row.id_tahun_ajaran}"><i class="fas fa-eye"></i></button>
                                <button class="btn btn-sm btn-warning penilaianBtn me-1" 
                                        ...
                                        ${row.status_periode === 0 || row.total_peserta_pbi === 0 ? 'disabled' : ''}>
                                    PENILAIAN
                                </button>
        ...
        $(document).on('click', '.lihatBtn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ url('guru/penilaian_pbi/data_list_periode_penilaian_kegiatan') }}/" + id_periode + "/" +
                        tahun_ajaran;
        ...
        $(document).on('click', '.penilaianBtn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ url('guru/penilaian_pbi/add_penilaian_kegiatan') }}/" + id_periode + "/" +
                        tahun_ajaran;
                    window.location.href = url;
                }
            });
